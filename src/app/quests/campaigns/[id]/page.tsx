...
function CampaignDetailContent({ paramsPromise }: { paramsPromise: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [campData, statusData, refStatsData] = await Promise.all([
  ...
  const myShareUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/quests/campaigns/${id}?ref=${encodeURIComponent(user?.displayName || 'traveler')}`
    : '';
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Pre-register for early-bird bounties and join the quest in Pangasinan:`,
          url: myShareUrl,
        });
      } catch (err) {
  ...
  const handleReserveSlot = async () => {
    if (!user) {
      window.location.href = `/login?redirect=/quests/campaigns/${id}`;
      return;
    }
    try {
      setReserving(true);
      const res = await reserveCampaignSlot(id, refCodeFromUrl || undefined);
      setUserStatus({
        isRegistered: true,
        ticketCode: res.ticketCode,
        isCompleted: false,
      });
      const [updatedCamp, updatedRefStats] = await Promise.all([
        fetchCampaignById(id),
        fetchCampaignReferralStats(id),
      ]);
      setCampaign(updatedCamp);
      setReferralStats(updatedRefStats);
    } catch (err: any) {
      alert(err.message || 'Failed to reserve ticket slot');
    } finally {
      setReserving(false);
    }
  };
  ...
  const handleClaimArrival = async () => {
    try {
      ...
      let lat = campaign?.gpsLat || 16.0;
      let lng = campaign?.gpsLng || 120.0;
      ...
      const res = await claimCampaignArrivalReward(id, lat, lng);
      setClaimSuccess(res.message || 'Check-in verified! Bounty credited to your wallet.');
      setUserStatus((prev) => (prev ? { ...prev, isCompleted: true } : null));
      ...
      alert(err.message || 'Failed to verify event check-in.');
    } finally {
      setClaiming(false);
    }
  };
    ...
    return (
      <Navigation>
        <div className="max-w-4xl mx-auto px-4 pt-16 text-center space-y-3">
          ...
          <p className="text-xs font-semibold text-gray-500">Loading campaign details & escrow allocations...</p>
        </div>
      </Navigation>
    );
  }
